                ...
                if name != None:
                    if obj.chkpassword(password) is True:
                        ...
                        if request.form.get("remember") == '1':
                            ...
                            rsp.set_cookie('auth', session[
                                           'auth'], expires=datetime.now() + timedelta(days=10))
                            return rsp
...
def add_post():
    if request.method == 'POST':
        try:
            ...
            post.insert().execute({'description': request.form[
                'body'], 'date_time': datetime.now()})
        except:
...
def post_update(id):
    if request.form['body'] != "":
        table = Table(session['auth'], metadata, autoload=True)
        table.update(table.c.id == id, values=(
            {'description': request.form['body']})).execute()
    return redirect('hello')
...
def search(q):
    ...
    jsn = [{"url": "", "display": ""}]
    table = Table('users', metadata, autoload=True)
    rs = select([table.c.id, table.c.uname],
                table.c.uname.like('%' + s + '%')).execute()
    for item in rs:
...
def tagsearch(p_id, q):
    ...
    for item in rs:
        ...
        if item.id != current_id:
            jsn.append({"post": p_id, "friend": "green" +
                        str(item.id), "display": item.uname})
...
def tag_info(post_id):
    ...
    tag_buffer = select([table.c.friends_id],
                        table.c.post_id == post_id).execute()
    taglist = ['None']
    empty = 'true'
...
def like_delete(post_id):
    like = Table('like', metadata, autoload=True)
    s = select([like.c.id]).where(like.c.post_id == post_id).where(
        like.c.user_id == session['auth']).execute()
    for row in s:
...
def like(post_id):
    ...
    rs = select([table.c.user_id], table.c.post_id ==
                post_id).order_by(table.c.id.desc()).execute()
    r = []
    type = 'unliked'
    you = ''
    comma = ''
...
def comment(post_id):
    ...
    rs = table.select(table.c.post_id == post_id).order_by(
        table.c.id.asc()).execute()
...
def add_comment():
    if request.method == 'POST':
        try:
            ...
            table = Table('comment', metadata, autoload=True)
            table.insert().execute({'post_id': p_id, 'comment': request.form[
                'body'], 'user_id': session['auth'], 'date_time': datetime.now()})
        except:
            flash('kk')
    p_id = request.form['p_id']
...
def utility_processor():
    ...
    def status(id, type):
        table = Table('users', metadata, autoload=True)
        if type == 'on':
            us = table.select(table.c.time_last_active >= (
                datetime.now() - timedelta(seconds=6))).execute()
    ...
    def online(id):
        uid = id.replace("green", "")
        table = Table('users', metadata, autoload=True)
        table.update(table.c.id == uid, values=(
            {'time_last_active': datetime.now()})).execute()
    ...
    def clearance(fid):
        ...
        table = Table('users', metadata, autoload=True)
        user_f_list = select(
            [table.c.friend], table.c.id == int(uid)).execute()
