...
class Friend():
    ...
    def add(self):
        ...
        query = select([self.table.c.out_friend_req],
                       self.table.c.id == int(self.u_id)).execute()
        previous_req = str(query.fetchone()[0])
        modified_req = str(previous_req + ";" + self.to_be_add_to_user)
        self.table.update(self.table.c.id == int(self.u_id), values=(
            {'out_friend_req': modified_req})).execute()

        # add user id to friends in_friend_req
        query = select([self.table.c.in_friend_req],
                       self.table.c.id == int(self.fid)).execute()
        previous_req = str(query.fetchone()[0])
        modified_req = str(previous_req + ";" + self.to_be_add_to_frnd)
        self.table.update(self.table.c.id == int(self.fid), values=(
            {'in_friend_req': modified_req})).execute()

    def check_friend(self):
        # chech if member is friend of current_user or others status
        query = select([self.table.c.friend, self.table.c.out_friend_req,
                        self.table.c.in_friend_req], self.table.c.id == int(self.u_id)).execute()
        for row in query:
    ...
    def accept_request(self):
        query = select([self.table.c.friend, self.table.c.in_friend_req],
                       self.table.c.id == int(self.u_id)).execute()
        f_query = select([self.table.c.friend, self.table.c.out_friend_req],
                         self.table.c.id == int(self.fid)).execute()
        for row in query:
            user_f = str(row[0])
        ...
        self.table.update(self.table.c.id == int(self.u_id), values=(
            {'friend': user_f_list, 'in_friend_req': user_in_list})).execute()
        self.table.update(self.table.c.id == int(self.fid), values=(
            {'friend': mem_f_list, 'out_friend_req': mem_out_list})).execute()

    def unfriend(self):
        query = select([self.table.c.friend], self.table.c.id ==
                       int(self.u_id)).execute()
        f_query = select([self.table.c.friend],
                         self.table.c.id == int(self.fid)).execute()
        ...
        self.table.update(self.table.c.id == int(self.u_id),
                          values=({'friend': flist})).execute()
        self.table.update(self.table.c.id == int(self.fid),
                          values=({'friend': f_flist})).execute()

    def cancel_request(self):
        query = select([self.table.c.out_friend_req],
                       self.table.c.id == int(self.u_id)).execute()
        f_query = select([self.table.c.in_friend_req],
                         self.table.c.id == int(self.fid)).execute()
        ...
        self.table.update(self.table.c.id == int(self.u_id),
                          values=({'out_friend_req': flist})).execute()
        self.table.update(self.table.c.id == int(self.fid),
                          values=({'in_friend_req': f_flist})).execute()

    def reject_request(self):
        query = select([self.table.c.in_friend_req],
                       self.table.c.id == int(self.u_id)).execute()
        f_query = select([self.table.c.out_friend_req],
                         self.table.c.id == int(self.fid)).execute()
        ...
        self.table.update(self.table.c.id == int(self.u_id),
                          values=({'in_friend_req': flist})).execute()
        self.table.update(self.table.c.id == int(self.fid),
                          values=({'out_friend_req': f_flist})).execute()
